...
.NET SDK для **Error Logs Service (ELS)** — управляемого SaaS централизованного сбора событий (от debug до fatal) с AI-диагностикой ошибок. Асинхронный батчинг, retry с экспоненциальным backoff, буферизация на диск, middleware для ASP.NET Core, провайдер для `Microsoft.Extensions.Logging`. Wire-формат совместим с [Go SDK](https://github.com/official-inso/els-go).
