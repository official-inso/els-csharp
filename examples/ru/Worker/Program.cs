// Generic Host worker: BackgroundService с захватом в ELS и корректным
// shutdown через встроенный IHostedService SDK.
...
internal sealed class PeriodicCheck : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (await timer.WaitForNextTickAsync(stoppingToken))
        {
            ...
            catch (Exception ex)
            {
                ...
                _log.LogError(ex, "тик {Tick} упал", tick);
            }
        }
    }
    ...
    {
        if (tick % 3 == 0) throw new InvalidOperationException($"синтетический сбой на тике {tick}");
