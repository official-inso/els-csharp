...
.NET SDK for the **Error Logs Service (ELS)** — a managed SaaS for centralised event logging (debug → fatal) with AI-assisted error triage. Asynchronous batching, retry with exponential backoff, disk-based buffering, ASP.NET Core middleware, `Microsoft.Extensions.Logging` provider — wire-compatible with the [Go SDK](https://github.com/official-inso/els-go).
